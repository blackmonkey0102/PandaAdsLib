...
public class RewardedAdManager: NSObject {
    ...
    public var loadingDialog: JGProgressHUD! = JGProgressHUD()
    ...
    public func loadRewardedAd(adPlacement: String, adUnitID: String, vc:UIViewController, isShowLoading: Bool, onSuccess: (() -> Void)? = nil, onFail: ((Error) -> Void)? = nil) {
        ...
        if isShowLoading {
            ...
            loadingTimeoutTimer = Timer.scheduledTimer(withTimeInterval: TimeInterval(TIME_OUT_LOADING), repeats: false) { [weak self] _ in
                    ...
                    Helpers.myLog(text: "Loading timeout reached")
        ...
        GADRewardedAd.load(withAdUnitID: adUnitID, request: GADRequest()) { [weak self] ad, error in
            ...
            if let error = error {
                Helpers.myLog(text: "Failed to load rewarded ad: \(error.localizedDescription)")
            ...
            Helpers.myLog(text: "Rewarded ad loaded successfully")
            self.rewardedAd = ad
    ...
    public func showRewardedAd(
        ...
        guard let rewardedAd = rewardedAd else {
            Helpers.myLog(text: "Rewarded ad is not ready to show")
            return
        }
...
//            Helpers.myLog(text: "User earned reward: \(reward.amount) \(reward.type)")
    ...
    public func showLoadingDialog(view:UIView, timeout: TimeInterval) {
        ...
        // Khởi tạo dialog nếu cần
        if loadingDialog == nil {
            loadingDialog = JGProgressHUD(style: .extraLight)
        }
        
        loadingDialog.textLabel.text = Bundle.localizedString(forKey: "loading_ads")
        loadingDialog.style = .extraLight
        loadingDialog.show(in: view, animated: true)
    ...
    public func dismissLoadingDialog(){
        ...
        if loadingDialog != nil && loadingDialog.isVisible {
...
extension RewardedAdManager: GADFullScreenContentDelegate {
    public func ad(_ ad: GADFullScreenPresentingAd, didFailToPresentFullScreenContentWithError error: Error) {
        Helpers.myLog(text: "Failed to present rewarded ad: \(error.localizedDescription)")
        AnalyticEvent.adsLogEvent(.ad_reward_show_failed)
    }

    public func adDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {
        Helpers.myLog(text: "Rewarded ad dismissed")
        rewardedAd = nil
    ...
    public func adDidRecordImpression(_ ad: GADFullScreenPresentingAd) {
        Helpers.myLog(text: "Rewarded ad recorded impression")
        StatusAds.isShowingInter = true
        AnalyticEvent.adsLogEvent(.ad_reward_open)
    }

    public func adDidRecordClick(_ ad: GADFullScreenPresentingAd) {
        Helpers.myLog(text: "Rewarded ad recorded click")

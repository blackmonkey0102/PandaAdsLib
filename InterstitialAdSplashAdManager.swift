...
public class InterstitialAdSplashAdManager: NSObject, GADFullScreenContentDelegate{
    ...
    public func loadAdAppOpen(idAdOpenApp: String, adPlacement: String, idAppOpenSplash: String, canShowAds: Bool, completion: @escaping (GADAppOpenAd?, Error?) -> Void) {
        ...
        if !canShowAds || MySettings.isRemoveAds() {
            Helpers.myLog(text: "\(adPlacement) config false, of remove ads")
            completion(nil, nil)
            return
        }
        ...
        GADAppOpenAd.load(withAdUnitID: idAdOpenApp, request: GADRequest(), orientation: .portrait) { ad, error in
            if let error = error {
                Helpers.myLog(text: "Failed to load AppOpen Splash ad: \(error.localizedDescription)")
    ...
    public func loadInterAdSplash(adPlacement: String, idInterSplash: String, canShowAds: Bool, completion: @escaping (GADInterstitialAd?, Error?) -> Void) {
        ...
        let timeoutWorkItem = DispatchWorkItem { [weak self] in
            if !isCompletionCalled {
                ...
                Helpers.myLog(text: "\(adPlacement) load timeout after 30 seconds")
        ...
        GADInterstitialAd.load(withAdUnitID: idInterSplash, request: request) { [weak self] interAd, error in
            if !isCompletionCalled{
                ...
                if let error = error {
                    Helpers.myLog(text: "Load \(adPlacement) failed: \(error.localizedDescription)")
                    ...
                    Helpers.myLog(text: "\(adPlacement) loaded successfully")
    ...
    public func ad(_ ad: GADFullScreenPresentingAd, didFailToPresentFullScreenContentWithError error: Error) {
        if isLoadAppOpen{
            Helpers.myLog(text: "AppOpen splash Ad failed to present full screen content: \(error.localizedDescription)")
            AnalyticEvent.adsLogEvent(.ad_open_show_failed)
            onAdDismissed?()
            return
        }
        Helpers.myLog(text: "\(adPlacement) failed to present full screen content: \(error.localizedDescription)")
        onAdDismissed?()
    ...
    public func adDidRecordImpression(_ ad: GADFullScreenPresentingAd) {
        if isLoadAppOpen{
            Helpers.myLog(text: "AppOpen splash Ad adDidRecordImpression.")
            AnalyticEvent.adsLogEvent(.ad_open_open)
            StatusAds.isShowingInter = true
            return
        }
        Helpers.myLog(text: "\(adPlacement) adDidRecordImpression")
        StatusAds.isShowingInter = true
        AnalyticEvent.adsLogEvent(.ad_inter_open)
    }

//    public func adDidPresentFullScreenContent(_ ad: GADFullScreenPresentingAd) {
//        Helpers.myLog(text: "Interstitial splash did present full screen content")
//        StatusAds.isShowingInter = true
//    }
//
    public func adDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {
        if isLoadAppOpen{
            Helpers.myLog(text: "AppOpen splash Ad did dismiss full screen content.")
        ...
        Helpers.myLog(text: "\(adPlacement) did dismiss full screen content")

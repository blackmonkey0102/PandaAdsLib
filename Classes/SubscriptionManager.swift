...
public class SubscriptionManager {
    ...
    public func checkPurchaseStatus(idSharedSecretInappPurchase: String, lifetimeProductId: String?, completion: @escaping ((Bool, Bool, String)?) -> Void) {
        guard let receiptURL = Bundle.main.appStoreReceiptURL,
            ...
            MyHelpers.myLog(text: "Không tìm thấy receipt")
    ...
    public func verifyReceipt(url: String, requestData: [String: Any], lifetimeProductId: String?, completion: @escaping ((Bool, Bool, String)?) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            guard let data = data, error == nil else {
                MyHelpers.myLog(text: "Lỗi khi kiểm tra receipt: \(error?.localizedDescription ?? "Unknown error")")
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    ...
                    if let status = json["status"] as? Int, status == 21007 {
                        MyHelpers.myLog(text: "Receipt từ Sandbox -> Chuyển sang Sandbox API")
                    ...
                    if let latestReceiptInfo = json["latest_receipt_info"] as? [[String: Any]] {
                        for transaction in latestReceiptInfo {
                            if let productId = transaction["product_id"] as? String {
                                MyHelpers.myLog(text: "🔥 Đã mua gói: \(productId)") // 🛠 In ra ID của subscription
                                ...
                                if let expiresDateMs = transaction["expires_date_ms"] as? String,
                                   let expiresDate = Double(expiresDateMs) {
                                    ...
                                    MyHelpers.myLog(text: "⏳ Hết hạn: \(expirationDate)")
                ...
                MyHelpers.myLog(text: "Lỗi khi parse JSON: \(error.localizedDescription). Response: \(String(data: data, encoding: .utf8) ?? "Không thể đọc dữ liệu")")
                DispatchQueue.main.async {

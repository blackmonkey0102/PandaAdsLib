...
public class InterstitialAdSplashAdManager: NSObject, GADFullScreenContentDelegate{
    ...
    // load AOA Splash
    public func loadAdAppOpen(adPlacement: String, idAppOpenSplash: String, canShowAds: Bool, completion: @escaping (GADAppOpenAd?, Error?) -> Void) {
        ...
        GADAppOpenAd.load(withAdUnitID: idAppOpenSplash, request: GADRequest(), orientation: .portrait) { ad, error in
            if let error = error {
